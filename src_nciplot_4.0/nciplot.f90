...
program nciplot
   ...
   ! cubes
   real*8, allocatable, dimension(:, :, :) :: crho, crho1, cgrad
   ...
   ! properties of rho
   real*8 :: rho, rho1, grad(3), dimgrad, grad2, hess(3, 3)
   ...
   real*8 :: sum_rhon_area(9), sum_rhon_vol_1(7)
   ...
   ! ONCI variable 
   real*8, allocatable :: csab(:,:,:,:), sum_sab_vol(:,:), orb_dens(:,:,:,:), sum_orb_vol(:,:)
   real*8, allocatable :: interaction_index(:,:), sum_gonci_vol(:)
   integer             :: orbl, orbp, checkpoint 
   real*8              :: maxvalue, dimplot, rho_sab_tot 
   logical             :: interorbital
   !===============================================================================!
...
12 continue    ! set grids from coarse to fine
   ...
   if (ind_g .eq. 1) then
      ...
      allocate (crho1(0:nstep(1) - 1, 0:nstep(2) - 1, 0:nstep(3) - 1), stat=istat)
      if (istat /= 0) call error('nciplot', 'could not allocate memory for density cube', faterr)
   ...
   if (ind_g .gt. 1) then
      ...
      allocate (tmp_crho(0:nstep(1) - 1, 0:nstep(2) - 1, 0:nstep(3) - 1), stat=istat)
      call move_alloc(tmp_crho, crho1)
...
20                continue
      ...
      if (.not. inter) then
         ...
         !$omp parallel do private (x,rho,rho1, grad,hess,heigs,hvecs,wk1,wk2,istat,grad2,&
         !$omp dimgrad,intra,rhom,flag,indx,i0,j0,k0) schedule(dynamic)
         do k = 0, nstep(3) - 1
            do j = 0, nstep(2) - 1
               do i = 0, nstep(1) - 1
...
! loop over multi-level grids
   ...
   if (interorbital) then
          call interorb(m(1), xinit, xinc, nstep, crho, cgrad, rhocut, dimplot, csab,interaction_index, checkpoint)   
          call write_interorb(rhocut, dimplot,oname, nstep, crho, cgrad, csab, checkpoint)
          call sort_index(interaction_index, checkpoint, maxvalue)
          call write_index(nstep, csab, interaction_index, checkpoint, maxvalue, ntotal, xinit, xinc, m(1), nfiles)
   endif 
   ...
   if (dointeg) then
      call dataGeom(sum_rhon_vol, sum_rhon_area, sum_signrhon_vol, xinc, nstep, crho,&
             crho_n, rmbox_coarse, nfiles)
   ...
   if (interorbital) then
      call orb_density(xinit, xinc, nstep, m, nfiles, orb_dens) 
      allocate(sum_orb_vol(0:2,m(1)%nmo))
      allocate(sum_sab_vol(0:2,0:checkpoint))
      sum_orb_vol=0
      call dataGeom_orb_inte(sum_orb_vol, xinc, nstep, m, orb_dens, rmbox_coarse, nfiles)
      print*, '------------------------------------------------------------------------'
      print*, '             INTEGRAL 1 ORBITAL DENSITY                                 '
      print*, '------------------------------------------------------------------------'
      do i=1, m(1)%nmo
        print*, 'INTEGRATION ORBITAL Nº', i
        print*, sum_orb_vol(0,i)
      enddo
      print*, '------------------------------------------------------------------------'
      call dataGeom_sab(sum_sab_vol, xinc, nstep, checkpoint, csab, rmbox_coarse, nfiles)
     print*, '------------------------------------------------------------------------'
      print*, '             INTEGRAL s2(ab) INTERACTIONS                              '
      print*, '------------------------------------------------------------------------'
      do i=0, checkpoint
        print*, 'INTEGRAL OF INTERACTION Nº', i+1
        print*, sum_sab_vol(0,i)
      enddo
      print*, '------------------------------------------------------------------------'
     allocate(sum_gonci_vol(0:checkpoint))
      call dataGeom_gonci(sum_gonci_vol, orb_dens, interaction_index, m, xinc, nstep, checkpoint,&
             csab, rmbox_coarse, nfiles)
     print*, '------------------------------------------------------------------------'
      print*, '             INTEGRAL GAMMA(onci) INTERACTIONS                              '
      print*, '------------------------------------------------------------------------'
      do i=0, checkpoint
        print*, 'INTEGRAL OF INTERACTION Nº', interaction_index(i,0)+1
        print*, sum_gonci_vol(i)
      enddo
      print*, '------------------------------------------------------------------------'
...
118 format('                                                     ', / &
          ...
          '                  ', / &
          '---------------------------------------------------------------------', /, &
          ' Integration  over the volumes of rho 1 orb                          '/, &
          '---------------------------------------------------------------------', /, &
          ' n=1.0           :', 3X, F15.8, /, &
          ' n=1.5           :', 3X, F15.8, /, &
          ' n=2.0           :', 3X, F15.8, /, &
...
! compute NCI geometry of the region enclosed by RDG isosurface
   subroutine dataGeom(sum_rhon_vol, sum_rhon_area, sum_signrhon_vol, xinc, nstep,&
                   crho, crho_n, rmbox_coarse, nfiles)
      ...
      real*8, intent(in) :: crho(0:nstep(1) - 1, 0:nstep(2) - 1, 0:nstep(3) - 1)
      real*8, intent(in) :: crho_n(0:nstep(1) - 1, 0:nstep(2) - 1, 0:nstep(3) - 1, 1:nfiles)
