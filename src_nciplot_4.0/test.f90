...
contains
   ...
   subroutine calcprops_wfn(xinit, xinc, n, mol, nmol, rho, rho1, grad, cheig)
      use reader
      use tools_io
      use tools_math
      use param
      ...
      real*8, dimension(n(1), n(2), n(3)), intent(out) :: rho, rho1, grad
      real*8, dimension(n(1), n(2), n(3)), intent(out) :: cheig
      real*8, allocatable, dimension(:, :) :: dx, dy, dz, d2, gg
      real*8, allocatable, dimension(:) :: tp, maxc, rhoaux, rhoaux1
      ...
      allocate (tp(n(1)), rhoaux(n(1)), rhoaux1(n(1)),  stat=istat)
      ...
      !$omp istat,rhoaux,rhoaux1) schedule(dynamic)
         ...
         do k = 0, n(3) - 1
            ...
            do m = 1, nmol
               ...
               do i = 0, n(1) - 1
                  ...
                  rhoaux1(ip)=rhoaux1(ip)+mol(m)%occ(18)*phi(18,1)*phi(18,1)
                  do imo = 1, mol(m)%nmo
            ...
            do ip = 1, n(1)
               ...
               rhoaux2(ip) = max(rhoaux1(ip), 1d-30)
               ...
               rho1(ip, jp, kp) = sign(rhoaux2(ip), heigs(2))*100d0
  ...
  subroutine phi_sab(x,m,l,p,phi)
    use reader

    type(molecule), intent(in) :: m
    real*8, intent(in) :: x(3)
    integer, intent(in):: l,p
    real*8, intent(out) :: phi(2,10)
    ...
    do i = 1, 10
       do ipri = 1, m%npri
             phi(1,i) = phi(1,i) + m%c(l,ipri)*pri(ipri,i)
             phi(2,i)= phi(2,i)+m%c(p,ipri)*pri(ipri,i)
          end do
       enddo


    end subroutine phi_sab
